  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500 font-serif italic mt-0.5">
              Calculate semester SGPA and converted percentage using the official formula: <code className="text-[#8B0000] font-mono font-bold bg-[#F5F5F5] px-1.5 py-0.5 border border-[#E2E2E2]">Percentage = (CGPA - 0.75) × 10</code>.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white border border-[#E2E2E2] rounded-sm p-4 sm:p-6 shadow-sm space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-black pb-2">
            ...
            <div className="flex items-center gap-1.5 text-xs">
              <span className="text-[10px] font-bold uppercase tracking-widest text-gray-500">Pre-fill:</span>
              {[1, 2, 3, 4, 5, 7].map(sem => (
                <button
                  key={sem}
                  onClick={() => loadPresetSemester(sem)}
                  className="px-2.5 py-1 rounded-sm bg-[#F5F5F5] hover:bg-gray-200 text-gray-800 text-[10px] font-bold uppercase tracking-wider border border-[#E2E2E2] transition-colors cursor-pointer"
                >
                  Sem {sem}
                </button>
              ))}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              <thead>
                <tr className="border-b border-[#E2E2E2] text-gray-600 bg-[#FAF9F6] font-bold uppercase tracking-wider text-[10px]">
                  ...
                  <tr key={sub.id} className="hover:bg-[#FAF9F6] transition-colors">
                    <td className="py-2 px-3">
                      <div className="space-y-1">
                        <input
                          ...
                          className="w-full bg-transparent font-serif font-semibold text-[#1A1A1A] focus:outline-none focus:bg-white px-1 py-0.5 rounded-sm border border-transparent focus:border-black"
                        />
                        <input
                          type="text"
                          value={sub.code}
                          onChange={(e) => handleUpdateSubject(sub.id, 'code', e.target.value)}
                          className="w-28 bg-[#F5F5F5] font-mono text-[10px] text-[#8B0000] font-bold focus:outline-none focus:bg-white px-1.5 py-0.5 rounded-sm border border-[#E2E2E2] uppercase"
                        />
                      </div>
                    </td>
                    ...
                    <td className="py-2 px-3">
                      <select
                        ...
                        className="w-full px-2 py-1.5 rounded-sm bg-[#FAF9F6] border border-[#E2E2E2] text-[#1A1A1A] font-semibold focus:outline-none focus:border-black cursor-pointer"
                      >
                        {[1, 2, 3, 4, 5].map((c) => (
                    ...
                    <td className="py-2 px-3">
                      ...
                      >
                        {AKTU_GRADING_SCALE.map((g) => (
                          <option key={g.gradeLetter} value={g.gradePoint}>
                    ...
                    <td className="py-2 px-3 text-center font-bold text-[#8B0000] font-mono text-sm">
                    ...
                    <td className="py-2 px-2 text-center">
                      <button
                        ...
                        className="p-1.5 rounded-sm text-gray-400 hover:text-[#8B0000] hover:bg-gray-100 disabled:opacity-20 transition-colors cursor-pointer"
                      >
          ...
          <div className="flex items-center justify-between pt-3 border-t border-[#E2E2E2]">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-sm bg-black hover:bg-gray-800 text-white text-xs font-bold uppercase tracking-wider shadow-sm cursor-pointer"
            >
            ...
            <span className="text-xs text-gray-600 font-mono">
              Total Credits: <strong className="text-black font-bold text-sm">{totalCredits}</strong>
            </span>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          {/* Main SGPA Scorecard */}
          <div className="bg-white border-l-4 border-[#8B0000] border-y border-r border-[#E2E2E2] rounded-sm p-5 shadow-sm space-y-4">
            ...
            <div className="flex items-baseline gap-3">
              <span className="font-serif text-5xl font-black text-[#1A1A1A] tracking-tight">
                {currentSgpa.toFixed(2)}
              </span>
              <span className="text-gray-400 text-sm font-serif italic">/ 10.00</span>
            </div>

            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[#E2E2E2] text-xs">
              <div className="p-3 rounded-sm bg-[#FAF9F6] border border-[#E2E2E2]">
                <span className="text-gray-500 block text-[9px] uppercase tracking-wider font-bold">AKTU Official %</span>
                <span className="font-serif text-lg font-black text-emerald-700">{aktuPercentage}%</span>
              </div>

              <div className="p-3 rounded-sm bg-[#FAF9F6] border border-[#E2E2E2]">
                <span className="text-gray-500 block text-[9px] uppercase tracking-wider font-bold">General % (×9.5)</span>
                <span className="font-serif text-lg font-black text-[#8B0000]">{standardPercentage}%</span>
              </div>
            </div>

            {/* Division Status */}
            <div className="p-3 rounded-sm bg-[#F5F5F5] border border-[#E2E2E2] text-xs space-y-1">
              <span className="font-bold text-[#1A1A1A] block uppercase tracking-wider text-[10px]">AKTU Division Estimation:</span>
              <p className="text-gray-800 font-serif italic">
                {currentSgpa >= 7.5 ? (
                  <span className="text-emerald-800 font-bold not-italic">
                    🏆 First Division with Honours (CGPA ≥ 7.5)
                  </span>
                ) : currentSgpa >= 6.0 ? (
                  <span className="text-[#1A1A1A] font-bold not-italic">
                    ✓ First Division (CGPA ≥ 6.0)
                  </span>
                ) : currentSgpa >= 5.0 ? (
                  <span className="text-amber-800 font-bold not-italic">
                    • Second Division (CGPA ≥ 5.0)
                  </span>
                ) : (
                  <span className="text-[#8B0000] font-bold not-italic">
          ...
          <div className="bg-white border border-[#E2E2E2] rounded-sm p-4 text-xs space-y-3 shadow-sm">
            <span className="font-serif text-sm font-bold text-[#1A1A1A] flex items-center gap-1.5 border-b border-black pb-1.5">
            ...
            <div className="grid grid-cols-2 gap-1.5 text-[11px]">
              {AKTU_GRADING_SCALE.map((g) => (
                <div
                  key={g.gradeLetter}
                  className="flex items-center justify-between p-2 rounded-sm bg-[#FAF9F6] border border-[#E2E2E2]"
                >
                  <span className="font-bold text-[#8B0000] font-mono">{g.gradeLetter}</span>
                  <span className="text-gray-600 font-mono text-[10px]">{g.gradePoint} pts</span>
                </div>
              ))}
            </div>
          </div>
